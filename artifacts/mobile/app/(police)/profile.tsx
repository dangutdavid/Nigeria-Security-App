      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: insets.top, paddingBottom: insets.bottom + 90 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.hero, { backgroundColor: PRIMARY }]}>
          ...
          <View style={[styles.roleChip, { backgroundColor: "rgba(255,255,255,0.18)" }]}>
            <Text style={styles.roleText}>{ROLE_LABELS[user?.role ?? "field_officer"] ?? user?.role}</Text>
          </View>
        ...
        <View style={[styles.statsRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
          {[
            { label: "My Cases", value: myReports.length },
            { label: "Open / Active", value: myOpen },
            { label: "Arrested", value: myArrested },
          ].map((s, i) => (
        ...
        <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          {[
            ...
            { label: "Agency", value: "Nigeria Police Force" },
            { label: "Command", value: user?.sector },
          ].map((f) => (
        ...
        <View style={[styles.menu, { backgroundColor: colors.card, borderColor: colors.border }]}>
          {menuItems.map((m, i) => (
            <TouchableOpacity key={m.label} style={[styles.menuItem, { borderBottomColor: colors.border, borderBottomWidth: i < menuItems.length - 1 ? 1 : 0 }]} onPress={m.onPress} activeOpacity={0.7}>
