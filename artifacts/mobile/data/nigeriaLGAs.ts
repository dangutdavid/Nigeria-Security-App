...
export const NIGERIA_STATE_LGAS: StateData[] = [
  {
    name: "Abia",
  ...
  {
    name: "Adamawa",
    lgas: ["Demsa","Fufure","Ganye","Gayuk","Gombi","Guyuk","Hong","Jada","Lamurde","Madagali","Maiha","Mayo Belwa","Michika","Mubi North","Mubi South","Numan","Shelleng","Song","Toungo","Yola North","Yola South"],
  },
  {
    name: "Akwa Ibom",
  ...
  {
    name: "Bauchi",
    lgas: ["Alkaleri","Bauchi","Bogoro","Damban","Darazo","Dass","Ganjuwa","Giade","Itas/Gadau","Jama'are","Katagum","Kirfi","Misau","Ningi","Shira","Tafawa Balewa","Toro","Warji","Zaki"],
  },
  {
    name: "Bayelsa",
  ...
  {
    name: "Kwara",
    lgas: ["Asa","Baruten","Edu","Ekiti","Ifelodun","Ilorin East","Ilorin South","Ilorin West","Irepodun","Isin","Kaiama","Moro","Offa","Oke Ero","Oyun","Patigi"],
  },
  {
    name: "Lagos",
...
export const ALL_STATE_NAMES = NIGERIA_STATE_LGAS.map((s) => s.name);

export function getLGAsForStates(stateNames: string[]): string[] {
  if (stateNames.length === 0) return [];
  const all: string[] = [];
  for (const s of NIGERIA_STATE_LGAS) {
    if (stateNames.includes(s.name)) {
      all.push(...s.lgas);
    }
  }
  return all.sort();
...
export function getAdjacentLGAs(stateName: string, lga: string): string[] {
  const stateData = NIGERIA_STATE_LGAS.find((s) => s.name === stateName);
  if (!stateData) return [];
  const idx = stateData.lgas.indexOf(lga);
  if (idx === -1) return stateData.lgas;
  const nearby: string[] = [];
  for (let i = Math.max(0, idx - 3); i <= Math.min(stateData.lgas.length - 1, idx + 3); i++) {
    if (i !== idx) nearby.push(stateData.lgas[i]);
  }
  return nearby;
}
